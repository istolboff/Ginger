...
void Main()
{
	...
	var hstr = GrammarEngine.sol_ListEntryForms(_engineHandle, 1074062734);
    var entryForms = Enumerable
        .Range(0, GrammarEngine.sol_CountStrings(hstr))
        .Select(i => GrammarEngine.sol_GetStringFX(hstr, i).ToLower(CultureInfo.GetCultureInfo("ru")))
        .AsImmutable()
		.Dump();
...
private sealed record AnnotatedWord();




IEnumerable<string> ExtractTildasToSeparateTokens(IEnumerable<string> tokens)
{
